...
module Handler.DB.RouteTextmessagerecipientsTextMessageRecipientSent where
...
postTextmessagerecipientsTextMessageRecipientIdSentR :: forall master. (
    YesodAuthPersist master,
    AuthEntity master ~ User,
    AuthId master ~ Key User,
    YesodPersistBackend master ~ SqlBackend)
    => TextMessageRecipientId -> HandlerT DB (HandlerT master IO) A.Value
postTextmessagerecipientsTextMessageRecipientIdSentR p1 = lift $ runDB $ do
    ...
    runDB_result <- do
        e2 <- do
            ...
            return $ e {
                            textMessageRecipientSent = (Just __currentTime)

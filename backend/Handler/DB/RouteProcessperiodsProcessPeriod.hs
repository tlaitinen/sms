...
module Handler.DB.RouteProcessperiodsProcessPeriod where
...
postProcessperiodsProcessPeriodIdR :: forall master. (
    YesodAuthPersist master,
    AuthEntity master ~ User,
    AuthId master ~ Key User,
    YesodPersistBackend master ~ SqlBackend)
    => ProcessPeriodId -> HandlerT DB (HandlerT master IO) A.Value
postProcessperiodsProcessPeriodIdR p1 = lift $ runDB $ do
    ...
    runDB_result <- do
        e2 <- do
            es <- select $ from $ \o -> do
                where_ (o ^. ProcessPeriodId ==. (val p1))
                limit 1
                return o
            e <- case es of
                [(Entity _ e')] -> return e'    
                _ -> sendResponseStatus status404 $ A.object [ 
                        "message" .= ("Could not update a non-existing ProcessPeriod" :: Text)
                    ]
        ...
        case vErrors of
             ...
             _ -> P.repsert p1 (e2 :: ProcessPeriod)

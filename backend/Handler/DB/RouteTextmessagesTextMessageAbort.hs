...
module Handler.DB.RouteTextmessagesTextMessageAbort where
...
postTextmessagesTextMessageIdAbortR :: forall master. (
    YesodAuthPersist master,
    AuthEntity master ~ User,
    AuthId master ~ Key User,
    YesodPersistBackend master ~ SqlBackend)
    => TextMessageId -> HandlerT DB (HandlerT master IO) A.Value
postTextmessagesTextMessageIdAbortR p1 = lift $ runDB $ do
    ...
    runDB_result <- do
        e2 <- do
            ...
            return $ e {
                            textMessageAborted = (Just __currentTime)
